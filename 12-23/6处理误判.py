...
def mainthread(posepath):
    ...
    dic_addweightPic = pixProbabilityMap(58,480,1,640) #获得整副图片的改了吗分布字典
...
#                drawrectangle(all_postlist[i+1], oldboxdisappeared[key])
                    searchArea = SearchRectangle(oldboxdisappeared[key])   #扩大的怀疑框的检索区域[x1,y1,x2,y2],(绝对坐标)
                    big_ROI = getScreenshot(all_postlist[i+1],searchArea)  #新图
                    cv2.imshow('big_ROI',big_ROI)
                    cv2.waitKey(10)
                    ROI = getScreenshot(all_postlist[i],oldboxdisappeared[key])   #获得上一帧的截图cv2格式图片
                    ...
                    width = ROI.shape[1]
                    height = ROI.shape[0]
                    for rows in range(0,(big_ROI.shape[0] - ROI.shape[0]),5):
                        for cols in range(0,(big_ROI.shape[1] - ROI.shape[1]),5):
...
#                            cv2.waitKey(100)
                    ...
                    nb = [flag_rows + searchArea[0],flag_cols + searchArea[1],flag_rows + searchArea[0] + width, flag_cols + searchArea[1] + height] 
                    drawrectangle(all_postlist[i+1],nb)
            ...
            continutDrawPic(all_postlist[i+1],allbox_dic,dic_addweightPic,drawPro=False,drawupdown=True,drawnum=True)
...
#                        print('allbox_dic[key][subkey]:',allbox_dic[key][subkey])
...
def SearchRectangle(oldboxpos):
    ...
    print('old:',oldboxpos)
    big_oldboxpos = []
    oldboxpos[0] = oldboxpos[0] - 40
    oldboxpos[1] = oldboxpos[1] -40
    oldboxpos[2] = oldboxpos[2] + 40
    oldboxpos[3] = oldboxpos[3] + 40
    if oldboxpos[0] < 2:
        oldboxpos[0] = 2
    if oldboxpos[1] < 2:
        oldboxpos[1] = 2
...
def getScreenshot(message, nbox):
    im = cv2.imread(message[0])
    nbox = [int(x) for x in nbox]
    ROI =im[nbox[1]:nbox[3],nbox[0]:nbox[2]]  #im[y1:y2,x1:x2]
    cv2.imshow('ROI',ROI)
    cv2.waitKey(10)
    return ROI

def drawrectangle(message,boxpos):
#    print('message[0]:',message[0])
#    print('boxpos:',boxpos)
    im = cv2.imread(message[0])
    cv2.rectangle(im, (int(boxpos[0]),int(boxpos[1])), (int(boxpos[2]),int(boxpos[3])), (255,255,255),2)
